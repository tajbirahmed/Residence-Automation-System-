            ...
            if (isset($_GET['hld'])) {
                ...
                while ($row = mysqli_fetch_assoc($resultq)) {
                    ...
                    $sql = "SELECT  SUM(amount) as total from `payment_history` where ApartmentId LIKE '$hld-%' and rent_of = '$month'";
                        $result = mysqli_query($con, $sql); 
                        $row = mysqli_fetch_assoc($result);
                        $total_earn = $row['total'];
                        $html = $html . '<tr><td></td><td></td><td></td><td style="text-align:center;">Total = '.$total_earn.'</td><td></td><td></td><td></td><td></td></tr>';
                ...
                $year = explode('-', $month);
                echo 'a';
                $html = $html . '<div class="container" style="page-break-before: always;">';
                ...
                $html = $html . '<tr>
                                    ...
                                    <td></td>
                                    <td></td>
                                    <td style="text-align:center;">Total = '.$total_expense.'</td>
                                    </tr>';
                ...
                $sql = "SELECT SUM(salary) as total_stuff_payment from staff where holdingNumber = $hld";
                $result = mysqli_query($con, $sql);
                if ($result){
                    $row = mysqli_fetch_assoc($result); 
                    $total_stuff_pay = $row['total_stuff_payment'];
                    $tot = $total_earn - $total_expense - $total_stuff_pay;
                    $html = $html . ' 
                                        <div class="container" style="page-break-before: always;">
                                        <h2 style="text-align:center;">Total Income Expenditure for '.$bname.' of '.$month_name.', '.$year[0].'</h2>
                                        <table class="table">
                                            <thead>
                                                <tr>
                                                <th scope="col" style="text-align: center;">Total Collection</th>
                                                <th scope="col" style="text-align: center;">Total Expense</th>
                                                <th scope="col" style="text-align: center;">Stuff Payment</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                <td style="text-align: center;">(+)'.$total_earn.'</td>
                                                <td style="text-align: center;">(-)'.$total_expense.'</td>
                                                <td style="text-align: center;">(-)'.$total_stuff_pay.'</td>
                                                </tr>
                                                <tr>
                                                    <td></td>
                                                    <td></td>
